...
export default function Hero({ posters = [] }: HeroProps) {
  ...
  function getColImages(colIndex: number): string[] {
    if (!activePosters || activePosters.length === 0) return [];
    const offset = colIndex * 2;
    const result: string[] = [];
    for (let i = 0; i < activePosters.length; i++) {
      result.push(activePosters[(i + offset) % activePosters.length]);
    }
